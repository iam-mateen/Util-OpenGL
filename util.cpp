...
// ------------------------------------------ Waiapp Developers ---------------------------------------------------------//
/** 
	Open Source (OpenGL Doucmentation for deep understanding)
	
	Shapes Implementation to be used with
	GLUT (OpenGL/(*)/+)
*/
#include "util.h"
...
void InitRandomizer()
{
	srand(time(nullptr)); // time(0) returns number of seconds elapsed since January 1, 2020.
}

// This function returns a random value within the specified range of [rmin,
// rmax] ...
long GetRandInRange(const long &rmin, const long &rmax)
{
	long range = rmax - rmin; // find the range
	long value =
		(rand() % (long)range) + rmin; // translate the generated number ...
									   //	cout << value << endl << flush;
	return value;
}

/*To draw a triangle we need three vertices with each vertex having
 * 2-coordinates [x, y] and a color for the triangle. This function takes 4
 * arguments first three arguments (3 vertices + 1 color) to draw the triangle
